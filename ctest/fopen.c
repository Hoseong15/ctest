...
/*
	fopen 모드
	r - 읽기 전용, 파일이 없거나 찾을 수 없는 경우 실패
	w - 쓰기 전용, 파일이 없으면 새로운 파일 생성, 기존의 파일 내용 삭제 다시 저장
	a - 추가쓰기 전용, 지정한 파일이 있으면 파일의 끝에 추가

	r+ - 읽고 쓰기 모드, 기존의 내용에 덮어쓰기, 파일이 없으면 새로 생성
	w+ - 읽고 쓰기 모드, 파일의 내용 삭제하고 다시 저장, 파일없으면 새로 생성
