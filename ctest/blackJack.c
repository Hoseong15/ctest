...
void play() {
	...
	int turn = 0;
	drawCard(player[0].player_card[0]->number,
		player[0].player_card[0]->shape);
	betting();
	int total_betting=0;
	...
	while (turn != 100) {
		...
		dealer(turn); // 딜러차례 - turn이 0이되면 딜러 턴 0이 아니면 패스
		turn = stayOrHit(turn); // 플레이어 차례 - turn이 1이면 첫번째 플레이어 턴
			// stayOrHit에서는 모든 플레이어가 카드를 받을지 안받을지 진행
			// 모두가 카드를 받지않으면 결과 출력하고 turn으로 반환 값 100
	}
}

static void draw(int total_betting) {
	//drawCard() 함수 실행이 되어야한다. 그외에 다른 내용도 화면 출력
	system("cls");
	...
	for (int i = 0; i < playerCnt; i++) {
		printf("\n-------------- %d 플레이어↓-----------------\n",player[i].num);
		for (int k = 0; k < player[i].player_card[k] != NULL; k++) {
